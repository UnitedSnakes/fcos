...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params["lateral3"] = nn.Conv2d(dummy_out_shapes[0][1][1], self.out_channels, 1)
        self.fpn_params["lateral4"] = nn.Conv2d(dummy_out_shapes[1][1][1], self.out_channels, 1)
        self.fpn_params["lateral5"] = nn.Conv2d(dummy_out_shapes[2][1][1], self.out_channels, 1)

        self.fpn_params["output3"] = nn.Sequential(
        ...
        self.fpn_params["output4"] = nn.Conv2d(self.out_channels, self.out_channels, 3, padding=1)
        self.fpn_params["output5"] = nn.Conv2d(self.out_channels, self.out_channels, 3, padding=1)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        # Replace "PASS" statement with your code
        p3_ = self.fpn_params["lateral3"](backbone_feats["c3"])
        p4_ = self.fpn_params["lateral4"](backbone_feats["c4"])
        p5_ = self.fpn_params["lateral5"](backbone_feats["c5"])

        fpn_feats["p5"] = self.fpn_params["output5"](p5_)

        fpn_feats["p4"] = self.fpn_params["output4"](p4_ + F.interpolate(fpn_feats["p5"], scale_factor=2, mode="nearest"))
        fpn_feats["p3"] = self.fpn_params["output3"](p3_ + F.interpolate(fpn_feats["p4"], scale_factor=2, mode="nearest"))
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        x_centers = (x_coords + 0.5) * level_stride
        y_centers = (y_coords + 0.5) * level_stride

        location_coords[level_name] = torch.stack([x_centers, y_centers], dim=-1).reshape(-1, 2)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while order.numel():
        ...
        width = torch.max(torch.tensor(0.0, device=boxes.device), inter_x2 - inter_x1)
        height = torch.max(torch.tensor(0.0, device=boxes.device), inter_y2 - inter_y1)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channels in stem_channels:
          ...
          nn.init.normal_(conv.weight, 0, 1e-2)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
            c = self.stem_cls(value)
            c = self.pred_cls(c)
            c = c.permute(0, 3, 2, 1)
            c = c.reshape(c.size(0), -1, c.size(-1))
            class_logits[key] = c

            b = self.stem_box(value)
            boxreg = self.pred_box(b)
            boxreg = boxreg.permute(0, 3, 2, 1)
            boxreg = boxreg.reshape(boxreg.size(0), -1, 4)
            boxreg_deltas[key] = boxreg

            ctr = self.pred_ctr(b)
            ctr = ctr.permute(0, 3, 2, 1)
            ctr = ctr.reshape(ctr.size(0), -1, 1)
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "PASS" statement with your code
    # print(gt_boxes)
    background_mask = (gt_boxes[:, :4] == -1).all(dim=1)
    # print(background_mask)

    left_deltas = locations[:, 0] - gt_boxes[:, 0]
    top_deltas = locations[:, 1] - gt_boxes[:, 1]
    # right_deltas = locations[:, 0] - boxes[:, 2]
    # bottom_deltas = locations[:, 1] - boxes[:, 3]
    right_deltas = gt_boxes[:, 2] - locations[:, 0]
    bottom_deltas = gt_boxes[:, 3] - locations[:, 1]

    deltas = torch.stack([left_deltas, top_deltas, right_deltas, bottom_deltas], dim=1)
    deltas /= stride

    # deltas = torch.full((locations.shape[0], 4), -1, dtype=locations.dtype, device=locations.device)
    # print(deltas)
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes = None
    # Replace "PASS" statement with your code
    background_mask = (deltas[:, :4] == -1).all(dim=1)
    masked_deltas = deltas.clone() * stride
    masked_deltas[background_mask] = 0
    
    x1 = locations[:, 0] - masked_deltas[:, 0]
    y1 = locations[:, 1] - masked_deltas[:, 1]
    x2 = locations[:, 0] + masked_deltas[:, 2]
    y2 = locations[:, 1] + masked_deltas[:, 3]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = (torch.min(left, right) * torch.min(top, bottom)) / (torch.max(left, right) * torch.max(top, bottom))
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        shape_per_fpn_level = {level: fpn_features[level].shape for level in fpn_features}
        ...
        # print(locations_per_fpn_level["p3"].shape) # 784, 2
        # input()
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "PASS" statement with your code        
        matched_gt_boxes = [
            fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_box) for gt_box in gt_boxes
        ]
        # print(matched_gt_boxes)
        # print("------------------------------------------------")
        # input()
        # batch, layer, N?, 5
        # print(len(matched_gt_boxes)) # 16
        # print(matched_gt_boxes[0]["p3"].shape) # torch.Size([784, 5])
        # print(matched_gt_boxes[0]["p4"].shape) # torch.Size([196, 5])
        # print(matched_gt_boxes[0]["p5"].shape) # torch.Size([49, 5])
        ...
        # print(matched_gt_boxes[0]["p3"][:, :4].shape)

        # batch, level, N?, 4
        matched_gt_deltas = [
            {
                level: fcos_get_deltas_from_locations(
                    locations=locations_per_fpn_level[level], # (784, 2)
                    gt_boxes=matched_boxes_per_image[level][:, :4], # (784, 4)
                    stride=self.backbone.fpn_strides[level]
                )
                for level in matched_boxes_per_image
            }
            for image_index, matched_boxes_per_image in enumerate(matched_gt_boxes)
        ]

        # print(matched_gt_deltas)
        # print("------------------------------------------------")
        # input()
        ...
        # Replace "PASS" statement with your code
        # print(matched_gt_boxes.shape) # 16, 1029, 5
        class_targets = matched_gt_boxes[:, :, 4].long()
        # print(class_targets.shape) # (16, 1029)
        background_mask = (class_targets == -1)
        class_targets[background_mask] = self.num_classes
        class_targets_one_hot = F.one_hot(class_targets, num_classes=self.num_classes + 1).float()
        class_targets_one_hot[background_mask] = 0.0
        class_targets_one_hot = class_targets_one_hot[:, :, :self.num_classes]
        loss_cls = sigmoid_focal_loss(pred_cls_logits, class_targets_one_hot, reduction="none")

        loss_cls[background_mask] = 0.0

        loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")
        loss_box[background_mask] = 0

        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)).view(-1, pred_ctr_logits.size(1))
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.squeeze(-1), gt_centerness, reduction="none")
        loss_ctr[background_mask] = 0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Replace "PASS" statement with your code
            # width, height = images.shape[2:] # batch size = 1
            height, width = images.shape[2:] # batch size = 1
            # print(height, width)
